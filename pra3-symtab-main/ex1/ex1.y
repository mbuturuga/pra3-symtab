...
sentence: EOL                                   {/* empty sentence */ $$ = NUL; }
    | aux_scope formula EOL aux_scope           { fprintf(stdout, GREEN "Valid formula at line %d:\n  %s\n\n" RESET, nlin, $2); $$ = NUL; free($2); }
...
composite_formula: 
      FORALL aux_scope VAR aux_var formula aux_end_scope %prec FORALL       { $$ = get_string("forall %s %s", $3, $5); free($3); free($5); }
    | EXISTS aux_scope VAR aux_var formula aux_end_scope %prec EXISTS       { $$ = get_string("exists %s %s", $3, $5); free($3); free($5); }
